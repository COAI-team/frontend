...
const AnalysisPageWithoutRag = () => {
    ...
    useEffect(() => {
        if (!isNew) {
            const loadAnalysis = async () => {
                try {
                    setIsLoading(true);
                    // This uses the standard getAnalysisResult because the result format is the same
    ...
    return (
        <div className="min-h-screen">
            {/* 상단 헤더 */}
            <div className="shadow-sm border-b bg-gray-50 dark:bg-gray-900">
                    ...
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-4">
                            ...
                            <h1 className="text-lg font-semibold text-purple-600">
                                {isNew ? '✨ 새 코드 분석 (No RAG Mode)' : '📊 분석 결과 리포트 (No RAG)'}
                            </h1>
                        </div>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    <div className="space-y-6">
                        {isNew && !analysisResult && !isLoading && (
                            <div className="rounded-lg shadow-sm border p-6">
                                <h2 className="text-lg font-semibold mb-4">⚙️ 분석 설정 (No RAG)</h2>
                                <div className="mb-4 p-3 bg-yellow-50 text-yellow-800 text-sm rounded border border-yellow-200">
                                    ⚠️ 이 모드는 RAG(검색 증강 생성)를 사용하지 않습니다. 과거 분석 이력을 참조하지 않고 순수하게 현재 코드만 분석합니다.
                                </div>
...
export default AnalysisPageWithoutRag;
