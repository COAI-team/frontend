...
const MistakeReportPage = () => {
    ...
    const [userAnswers, setUserAnswers] = useState({}); // { id: boolean }
    const [quizResult, setQuizResult] = useState(null); // 'PASS' or 'FAIL'
    ...
    const handleSubmitQuiz = async () => {
        ...
        // Check if all answers match the correct 'answer' boolean in the data
        ...
        if (correctCount === questions.length) {
            try {
                ...
                // Even if server fails, locally show fail to be safe or pass if intended
                // user cleared the quiz logic wise.
                setQuizResult('FAIL');
            }
        } else {
            setQuizResult('FAIL');
    ...
    const handleResultConfirm = () => {
        ...
        if (quizResult === 'PASS') {
            ...
            setQuizStarted(false); // Go back to report to re-study
            setUserAnswers({}); // Clear answers for next attempt
        }
    };

    if (loading) return <div className="flex justify-center items-center h-screen text-2xl font-bold animate-pulse text-red-600">선생님이 생활기록부를 뒤적거리는 중...</div>;
    if (error) return <div className="text-center mt-20 text-red-500 font-bold text-xl">{error}</div>;
    if (!reportData) return <div className="min-h-screen bg-gray-900 text-white flex justify-center items-center">No Report Data Found.</div>;
        ...
        <div className="min-h-screen bg-gray-900 text-gray-100 p-8 font-mono relative overflow-hidden">
            ...
            <div className="max-w-4xl mx-auto relative z-10">
                ...
                <div className="mb-8 border-b-4 border-red-600 pb-4">
                    <h1 className="text-4xl md:text-5xl font-black text-red-500 mb-2 uppercase tracking-tighter drop-shadow-[0_0_10px_rgba(220,38,38,0.8)]">
                        {title || "WAKE UP CALL"}
                    </h1>
                            ...
                            <div className="mb-12">
                                ...
                                <div className="space-y-6">
                                    {mistakeDetails.map((detail, idx) => (
                                        <div key={idx} className="bg-gray-800/50 border border-yellow-600/30 rounded-lg p-6 relative overflow-hidden group hover:border-yellow-500/80 transition-all">
                ...
                <AlertModal
                    ...
                    type={quizResult === 'PASS' ? "success" : "danger"}
                    title={quizResult === 'PASS' ? "통과 (PASS)" : "불합격 (FAIL)"}
                    message={
                        quizResult === 'PASS' 
                        ? "좋아, 이제 좀 사람 같네. 이번엔 봐주겠지만... 다음에 또 그러면 그땐 진짜 파묻는다. (경고가 해제되었습니다)"
                        : "장난하냐? 아직도 정신 못 차렸네. 다시 공부하고 와."
                    }
                    confirmText={quizResult === 'PASS' ? "채굴장으로 복귀" : "처음부터 다시 하기"}
                />
            </div>
        </div>
    );
